...
import { getUsersU, createUserU, updateUserU, activateUserU } from '../services/userU';
...
const useUserU = () => {
  ...
  const getAllUsers = async () => {
    try {
        ...
        const filteredUsers = userData.filter(user => user.ID_ROL_USUARIO === 2);
  ...
  const openModal = (op, id_usuario = '', nombre1_usuario = '', nombre2_usuario = '', 
    ...
    setIdRolUsuario(op === 2 ? '2' : id_rol_usuario);
    setOperation(op);
  ...
  const handleToggleCuenta = async (id_usuario, nuevoEstado) => {
    try {
        ...
        await activateUserU(id_usuario, estadoNumerico);
