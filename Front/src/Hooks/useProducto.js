...
import { 
    ...
    updateProducto, 
    deleteProducto, 
    activarProducto } from '../services/producto.js';
...
export const useProducto = () => {
  ...
  const fetchAllProductos = async (id_usuario) => {
    try {
      ...
      const sortedProductos = productosData.sort((a, b) => a.ID_PRODUCTO - b.ID_PRODUCTO);
    ...
    try {
      ...
      setProductos((prevProductos) =>
        prevProductos.map((item) =>
          item.ID_PRODUCTO === id_producto ? { ...item, NOMBRE_PRODUCTO: nombre_producto.nombre_producto } : item
        )
      );
    } catch (error) {
    ...
    try {
      ...
      setProductos((prevProductos) =>
        prevProductos.map((producto) =>
          producto.ID_PRODUCTO === id_producto ? { ...producto, ESTADO_PRODUCTO: nuevoEstado } : producto
        )
      );
      const mensaje = nuevoEstado ? 'Producto activado con éxito' : 'Producto desactivado con éxito';
